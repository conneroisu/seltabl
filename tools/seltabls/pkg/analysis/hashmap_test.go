...
func TestMapPut(t *testing.T) {
	...
	assert.Same(t, m.Keys(), []int{1, 2, 3, 4, 5, 6, 7})
	assert.Same(t, m.Values(), []string{"a", "b", "c", "d", "e", "f", "g"})
...
func TestMapRemove(t *testing.T) {
	...
	assert.Same(t, m.Keys(), []int{1, 2, 3, 4})
	assert.Same(t, m.Values(), []string{"a", "b", "c", "d"})
	...
	assert.Same(t, m.Keys(), nil)
	assert.Same(t, m.Values(), nil)
...
func TestMapSerialization(t *testing.T) {
	...
	var err error
	exp := func() {
		assert.Same(t, m.Keys(), []string{"a", "b", "c"})
		assert.Same(t, m.Values(), []float64{1.0, 2.0, 3.0})
